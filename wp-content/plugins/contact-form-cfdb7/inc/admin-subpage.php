...
class CFDB7_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        $cfdb        = apply_filters( 'nasa_database', $wpdb );
        $table_name  = $cfdb->prefix.'nasa';
    ...
    public function get_bulk_actions() {
        ...
        return array(
            'read'   => __( 'Read', 'configuracion-reporte-nasa' ),
            'unread' => __( 'Unread', 'configuracion-reporte-nasa' ),
            'delete' => __( 'Delete', 'configuracion-reporte-nasa' )
        );
    ...
    protected function bulk_actions( $which = '' ) {
        ...
        echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . __( 'Select bulk action', 'configuracion-reporte-nasa' ) . '</label>';
        echo '<select name="action' . $two . '" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
        echo '<option value="-1">' . __( 'Bulk Actions', 'configuracion-reporte-nasa' ) . "</option>\n";
        ...
        submit_button( __( 'Apply', 'configuracion-reporte-nasa' ), 'action', '', false, array( 'id' => "doaction$two" ) );
        echo "\n";
        $nonce = wp_create_nonce( 'dnonce' );
        echo "<a href='".$_SERVER['REQUEST_URI']."&csv=true&nonce=".$nonce."' style='float:right; margin:0;' class='button'>";
        _e( 'Export CSV', 'configuracion-reporte-nasa' );
        echo '</a>';
